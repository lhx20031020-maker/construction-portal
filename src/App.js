...
const inputStyle = { width: '100%', maxWidth: '100%', boxSizing: 'border-box', padding: '12px', paddingLeft: '12px', borderRadius: '8px', border: `1px solid ${THEME.border}`, backgroundColor: '#fafafa', fontWeight: '600', color: THEME.sidebar, outline: 'none' };
...
const [lastSaved, setLastSaved] = useState("Today, 08:15 AM");
const handleGlobalSave = () => {
  ...
  // Logic to actually persist your labour changes would go here
  alert("Changes synced to project database!");
};
 const handleLogin = () => {
 ...
 // helper: product cost for a single task name
 const getTaskProductCost = (taskName) =>
  libraryData
   .filter(i => i.Task === taskName && i.Category === 'Product')
   .reduce((sum, item) => sum + (quantities[item.Code] || 0) * item["Price (€)"], 0);

 // helper: baseline labour cost for a single task name (1 person, 6 hours)
 const getTaskLabourCost = (taskName) =>
  libraryData
   .filter(i => i.Task === taskName && i.Category === 'Labor')
   .reduce((sum, item) => sum + (1 * 6 * item["Price (€)"]), 0);

// grand total across all phases/tasks (Products + Labour Baseline)
  const grandTotal = useMemo(() => {
  const allTasks = Object.values(PROJECT_MAP).flat();
  const productSum = allTasks.reduce((t, task) => t + getTaskProductCost(task), 0);
  const labourSum = allTasks.reduce((t, task) => t + getTaskLabourCost(task), 0);
  return productSum + labourSum;
 }, [quantities]);
 ...
 const phasesWithCosts = useMemo(() =>
  phases.map(phase => ({
   ...
   totalCost: (PROJECT_MAP[phase.name] || []).reduce(
    (sum, task) => sum + getTaskProductCost(task) + getTaskLabourCost(task), 0
   )
  })),
 [quantities]);
